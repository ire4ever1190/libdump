## Utilies for writing documentation
## Thrown in so docs don't need a full package
...
template initExampleBlock*(p: string, compileArgs: string): ExampleBlock =
  ## Creates a new example block that can be referenced for other examples
  runnableExamples:
    const mainExample = initExampleBlock("foo")

    # Code is carried between blocks
